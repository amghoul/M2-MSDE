...
def train(args,dataloader, model, optimizer, log, float_model_dict=None,epoch=1):
    ...
    for batch_idx, (imgL, imgR, disp_L, disp_L_occ_noc, obj_map_img,left_path,right_path) in enumerate(dataloader):
        ...
        '''
        if args.loss_name == "smooth":
            loss = [args.loss_weights[0] * smooth_L1_loss(outputs[0],disp_L, args)]
        else:
            loss = [args.loss_weights[0] * GERF_loss(outputs[0],disp_L, args)]
        epe_list = [epe(outputs[0], disp_L, args)]
        for i in range(len(outputs)-1):
            if args.loss_name == "smooth":
                loss.append(args.loss_weights[i+1] * smooth_L1_loss(outputs[i+1],disp_L, args))
            else:
                loss.append(args.loss_weights[i+1] * GERF_loss(outputs[i+1],disp_L, args))

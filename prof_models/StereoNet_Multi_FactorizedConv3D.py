...
from tempmodels.factorizer import Factorizer
from tempmodels.spatioTemporalConv_General import SpatioTemporalConv
from tempmodels.fpn import FPN, LayerScale
from tempmodels.cv import initCV, res_and_rec_CV
import time
###############
...
comp_names={"fe":0,"cv0":1,"cv1":2,"cv2":3,"cv3":4,"cf0":5,"cf1":6,"cf2":7,"cf3":8,"st0":9,"st1":10,"st2":11,"st3":12,"ft":13}
com_fullNames=["Feature_Extraction", "Cost_Volume_stage0", "Cost_Volume_stage1","Cost_Volume_stage2","Cost_Volume_stage3",
    "Cost_Filtering_stage0","Cost_Filtering_stage1","Cost_Filtering_stage2","Cost_Filtering_stage3",
    "Stage0","Stage1","Stage2","Stage3","Full_time"]
...
class StereoNet(nn.Module):
    def __init__(self, r, initial_ch, num_convs_in_layers, initial_scale_factor, disp_offset,sub_pixel_acc,patch_index,is_costvolume_4D,BN_2D_last, chout_costfiltring, is_filter1_differ,filter1_kernels,fact_kernels,BN_1D,BN_2D,BN_1D_last,model_bn,maxdisp=192):
        super().__init__()
    ...
    def forward(self, left, right):
        ...
        if self.initial_scale_factor == 4:
            scale_layer_refimg=[16,8,4]
        if self.initial_scale_factor == 2:
            scale_layer_refimg=[8,4,2]
        top_down_refimg_feature = self.feature_extraction(left) # top_down_refimg_feature =[torch.Size([7, 32, 34, 60],torch.Size([7, 16, 68, 120], torch.Size([7, 8, 135, 240]]
        top_down_targetimg_feature = self.feature_extraction(right) # scale_layer_refimg = [16, 8, 4]
...
class StereoNetProf(nn.Module):
    ...
    #@profile
    def forward(self, left, right):
        _,_,img_h,img_w = left.size() 
        if self.initial_scale_factor == 4:
            if len(self.num_convs_in_layers) == 3:
                scale_layer_refimg=[16,8,4]
            else:
                scale_layer_refimg=[32,16,8,4]
        if self.initial_scale_factor == 2:
            scale_layer_refimg=[8,4,2]
        top_down_refimg_feature = self.feature_extraction(left)
        top_down_targetimg_feature = self.feature_extraction(right) 
        #top_down_refimg_feature, scale_layer_refimg = self.feature_extraction(left) 
        #top_down_targetimg_feature, scale_layer_targetimg = self.feature_extraction(right)
        pred_list=[]
        pred_org_size_list=[]
        scale_ind=0
        f_h, f_w = top_down_refimg_feature[scale_ind].size(2), top_down_refimg_feature[scale_ind].size(3)
        ...
        prev_disp_org_size= pred_org_size_list[scale_ind-1]
        prev_disp_org_size = prev_disp_org_size * f_w / prev_disp_org_size.size(-1)
        prev_disp_up = F.interpolate(prev_disp_org_size,size=(f_h, f_w),mode='bilinear', align_corners=True) 
        prev_disp_up = torch.squeeze(prev_disp_up,1)
        cost = self.res_rec_cv(top_down_refimg_feature[scale_ind], top_down_targetimg_feature[scale_ind], prev_disp_up,self.disp_offset,self.sub_pixel_acc) 
        cost = torch.unsqueeze(cost, 1)
        cost = self.filter[scale_ind](cost) 
        cost = torch.squeeze(cost, 1) 
        pred =soft_argmin(cost)
        ...
        sum_res_disp = pred_low_res + torch.unsqueeze(prev_disp_up,1)
        pred_org_size_list.append(sum_res_disp)
        if len(self.num_convs_in_layers) == 4:
            scale_ind=3
            f_h, f_w = top_down_refimg_feature[scale_ind].size(2), top_down_refimg_feature[scale_ind].size(3)
            prev_disp_org_size= pred_org_size_list[scale_ind-1]
            prev_disp_org_size = prev_disp_org_size * f_w / prev_disp_org_size.size(-1)
...
class StereoNetTime(nn.Module):
    ...
    def forward2(self, left, right):
        ...
        for scale_ind in range(len(scale_layer_refimg)):
            ...
            if scale_ind == 0:
                # cost volume 0
                starter_cv0= time.perf_counter()
                cost = self.initcv(top_down_refimg_feature[scale_ind], top_down_targetimg_feature[scale_ind], scale_layer_refimg[scale_ind])
                torch.cuda.synchronize()
                ender_cv0= time.perf_counter()
                curr_time = ender_cv0-starter_cv0
                timings_components[comp_names["cv0"]]+= curr_time

            else:
                if scale_ind == 1:
                    # cost volume 1
                    starter_cv1= time.perf_counter()
                if scale_ind == 2:
                    # cost volume 2
                    starter_cv2= time.perf_counter()
                if scale_ind == 3:
                    # cost volume 3
                    starter_cv3= time.perf_counter()
                prev_disp_org_size= pred_org_size_list[scale_ind-1]
                prev_disp_org_size = prev_disp_org_size * f_w / prev_disp_org_size.size(-1)
                prev_disp_up = F.interpolate(prev_disp_org_size,size=(f_h, f_w),mode='bilinear', align_corners=True) 
                prev_disp_up = torch.squeeze(prev_disp_up,1)
                ...
                if scale_ind == 1:
                    # cost volume 1
                    ender_cv1= time.perf_counter()
                    curr_time = ender_cv1-starter_cv1
                    timings_components[comp_names["cv1"]]+= curr_time
                if scale_ind == 2:
                    # cost volume 2
                    ender_cv2= time.perf_counter()
                    curr_time = ender_cv2-starter_cv2
                    timings_components[comp_names["cv2"]]+= curr_time
                if scale_ind == 3:
                    # cost volume 3
                    ender_cv3= time.perf_counter()
                    curr_time = ender_cv3-starter_cv3
                    timings_components[comp_names["cv3"]]+= curr_time

            if self.is_costvolume_4D == 1:
                cost = torch.unsqueeze(cost, 1)
                if scale_ind == 0:
                    # cost filter 0
                    starter_cf0= time.perf_counter()
                if scale_ind == 1:
                    # cost filter 1
                    starter_cf1= time.perf_counter()
                if scale_ind == 2:
                    # cost filter 2
                    starter_cf2= time.perf_counter()
                if scale_ind == 3:
                    # cost filter 3
                    starter_cf3= time.perf_counter()
                ...
                if scale_ind == 0:
                    # cost filter 0
                    ender_cf0= time.perf_counter()
                    curr_time = ender_cf0-starter_cf0
                    timings_components[comp_names["cf0"]]+= curr_time
                if scale_ind == 1:
                    # cost filter 1
                    ender_cf1= time.perf_counter()
                    curr_time = ender_cf1-starter_cf1
                    timings_components[comp_names["cf1"]]+= curr_time
                if scale_ind == 2:
                    # cost filter 2
                    ender_cf2= time.perf_counter()
                    curr_time = ender_cf2-starter_cf2
                    timings_components[comp_names["cf2"]]+= curr_time
                if scale_ind == 3:
                    # cost filter 3
                    ender_cf3= time.perf_counter()
                    curr_time = ender_cf3-starter_cf3
                    timings_components[comp_names["cf3"]]+= curr_time
            else: # is_costvolume_4D == 0
                if scale_ind == 0:
                    # cost filter 0
                    starter_cf0= time.perf_counter()
                if scale_ind == 1:
            ...
            if scale_ind==0:
                ...
                #starter_st= time.perf_counter()
                if scale_ind == 1:
                    # stage 1
                    starter_st1= time.perf_counter()
                if scale_ind == 2:
                    # stage 2
                    starter_st2= time.perf_counter()
                if scale_ind == 3:
                    # stage 3
                    starter_st3= time.perf_counter()
                ...
                if scale_ind == 2:
                    # stage 2
                    ender_st2= time.perf_counter()
                    curr_time = ender_st2-starter_st2
                    timings_components[comp_names["st2"]]+= curr_time 
                if scale_ind == 3:
                    # stage 3
                    ender_st3= time.perf_counter()
                    curr_time = ender_st3-starter_st3
    ...
    if toProfile == "timeOnly":
        ...
        for i in range(num_components):
            if i == num_components - 1:
                print("Other_Time",": ", "{:.3f}".format((mean_tim_comp[i]-sum(mean_tim_comp[:i]))*1000)," ms" , "perc: ", "{:.3f}".format((mean_tim_comp[i]-sum(mean_tim_comp[:i])) * 100/mean_tim_comp[num_components - 1]),"%")
                print("Actual_Time",": ", "{:.3f}".format(sum(mean_tim_comp[:i]) * 1000)," ms" )
                print(com_fullNames[i],": ", "{:.3f}".format(mean_tim_comp[i]*1000), " ms")
            else:
                print(com_fullNames[i],": ", "{:.3f}".format(mean_tim_comp[i]*1000), " ms", "perc: ", "{:.3f}".format(mean_tim_comp[i] * 100/mean_tim_comp[num_components - 1]),"%")
        torch.cuda.synchronize()
...
def print_model_info(model_bn,is_filter1_differ,BN_1D_last,modeltype,fact_kernels,BN_1D,BN_2D):
        #########  
        if model_bn == 1:
            if is_filter1_differ == 1:
                if BN_1D_last == 0:
                    print('========= Model ' , modeltype , ' was loaded (filter 1 is differ) with number of subspaces is ', str(len(fact_kernels)+len(filter1_kernels)-1) , ' + last Conv1D, BN_1D is ' , str(BN_1D), ' and BN_2D is ' , str(BN_2D))  
                else:
                    print('========= Model ' , modeltype , ' was loaded (filter 1 is differ) with number of subspaces is ', str(len(fact_kernels)+len(filter1_kernels)-1) , ' + last Conv1D, BN enabled for the last conv1D only')  
                print("The used kernels are: ", "filter1 kernels: ",filter1_kernels, ", Kernels for the remaining filters are: ",fact_kernels)
            else:
                if BN_1D_last == 0:
                    print('========= Model ' , modeltype , ' was loaded with number of subspaces is ', str(len(fact_kernels)-1) , ' + last Conv1D, BN_1D is ' , str(BN_1D), ' and BN_2D is ' , str(BN_2D))
                else:
                    print('========= Model ' , modeltype , ' was loaded with number of subspaces is ', str(len(fact_kernels)-1) , ' + last Conv1D, BN enabled for the last conv1D only')
                print("The used kernels are: ", fact_kernels)
        else:
            if is_filter1_differ == 1:
                print('========= Model ' , modeltype , ' was loaded (filter 1 is differ) with number of subspaces is ', str(len(fact_kernels)+len(filter1_kernels)-1) , ' + last Conv1D, with no Batch Normalized used for whole the model')  
                print("The used kernels are: ", "filter1 kernels: ",filter1_kernels, ", Kernels for the remaining filters are: ",fact_kernels)
            else:
                print('========= Model ' , modeltype , ' was loaded with number of subspaces is ', str(len(fact_kernels)-1) , ' + last Conv1D, with no Batch Normalized used for whole the model')
                print("The used kernels are: ", fact_kernels)
...
if __name__ == '__main__':
    ...
    #fact_kernels=[[3, 3, 1], [1, 3, 3], [3, 1, 1]] #[[3,3,1], [1,1,3],[1,1,3]]
    ...
    #initial_scale_factor_list = [2,4]
    #sub_pixel_acc_list = [1.0,0.5]
    #chout_costfiltring_list = [1,32]
    initial_scale_factor_list = [4]
    sub_pixel_acc_list = [1.0]
    chout_costfiltring_list = [32]
    
    stages = 4
    #num_convs_in_layers = [2,2,2]
    num_convs_in_layers = [2,2,2,2]
    #prof_list=["memlab"]
    ...
    B,C,H,W = dummy_input.size()
    model_name = "nv-sn-v04-ab13-4scales"
    
    for initial_scale_factor in initial_scale_factor_list:
        for chout_costfiltring in chout_costfiltring_list:
            for sub_pixel_acc in sub_pixel_acc_list:
                modeltype = model_name+"_stages_"+str(stages)
                print("!!!!!!!! Profiling stage: " , str(stages), ", initial_scale_factor: ", initial_scale_factor, ", chout_costfiltring: ", chout_costfiltring, "sub_pixel_acc: ", sub_pixel_acc )
                print_model_info(model_bn,is_filter1_differ,BN_1D_last,modeltype,fact_kernels,BN_1D,BN_2D)
                model = StereoNet(r=stages, initial_ch=8, num_convs_in_layers=num_convs_in_layers, initial_scale_factor=initial_scale_factor, disp_offset=2,sub_pixel_acc=sub_pixel_acc,patch_index=2,
                    is_costvolume_4D=1,BN_2D_last=0, chout_costfiltring=chout_costfiltring, is_filter1_differ=is_filter1_differ,filter1_kernels=filter1_kernels, fact_kernels=fact_kernels,
                    BN_1D=BN_1D,BN_2D=BN_2D,BN_1D_last=BN_1D_last,model_bn=model_bn).to(device)
                model_time = StereoNetTime(r=stages, initial_ch=8, num_convs_in_layers=num_convs_in_layers, initial_scale_factor=initial_scale_factor, disp_offset=2,sub_pixel_acc=sub_pixel_acc,patch_index=2,
                    is_costvolume_4D=1,BN_2D_last=0, chout_costfiltring=chout_costfiltring, is_filter1_differ=is_filter1_differ,filter1_kernels=filter1_kernels, fact_kernels=fact_kernels,
                    BN_1D=BN_1D,BN_2D=BN_2D,BN_1D_last=BN_1D_last,model_bn=model_bn).to(device)
                model_memlab = StereoNetProf(r=stages, initial_ch=8, num_convs_in_layers=num_convs_in_layers, initial_scale_factor=initial_scale_factor, disp_offset=2,sub_pixel_acc=sub_pixel_acc,patch_index=2,
                    is_costvolume_4D=1,BN_2D_last=0, chout_costfiltring=chout_costfiltring, is_filter1_differ=is_filter1_differ,filter1_kernels=filter1_kernels, fact_kernels=fact_kernels,
                    BN_1D=BN_1D,BN_2D=BN_2D,BN_1D_last=BN_1D_last,model_bn=model_bn).to(device)
                
                for toProfile in prof_list:
                    prof_model(model, model_time, model_memlab, stages, dummy_input,toProfile,modeltype)

...
class myImageFloder(data.Dataset):
    def __init__(self, left, right, left_disparity,left_disp_noc, mask_obj_map, training,flip_vertical, loader=default_loader, dploader= disparity_loader):
    ...
    def __getitem__(self, index):
        ...
        if self.training:  
            ...
            mask_obj_map_img = np.ascontiguousarray(mask_obj_map_img,dtype=np.float32)/256
            mask_obj_map_img = mask_obj_map_img[y1:y1 + th, x1:x1 + tw]
            ...
            dataL_noc = np.ascontiguousarray(dataL_noc,dtype=np.float32)/256
            mask_obj_map_img = mask_obj_map_img.crop((w-1232, h-368, w, h))
            mask_obj_map_img = np.ascontiguousarray(mask_obj_map_img,dtype=np.float32)/256
